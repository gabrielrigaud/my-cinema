...
async function loadSeances(page = 1) {
    try {
        ...
        const response = await api.getScreenings(params);
        const seances = response.data.screenings;
        ...
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">Erreur lors du chargement des séances</td></tr>';
    }
}
...
function renderSeances(seances) {
    ...
    if (!seances || seances.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">Aucune séance trouvée</td></tr>';
        return;
    }
        ...
        tr.innerHTML = `
            <td>
                <div class="seance-film">
                    <strong>${escapeHtml(seance.movie_title)}</strong>
                    <br><small class="text-muted">${seance.movie_duration} min</small>
                </div>
            </td>
            <td>
                <div class="seance-salle">
                    <strong>${escapeHtml(seance.room_name)}</strong>
                    <br><small class="text-muted">${seance.room_type} - ${seance.room_capacity} places</small>
                </div>
            </td>
            <td>
                <span class="badge badge-date">${formatDate(seance.start_time)}</span>
            </td>
            <td>
                <span class="badge badge-time">${formatTime(seance.start_time)}</span>
            </td>
            <td>
...
function confirmDeleteSeance(seanceId) {
    openConfirmModal(
        ...
        async () => {
            try {
                await api.deleteScreening(seanceId);
